...
public:
	...
	std::string to_string( bool with_edges = false ) {
		std::string ret = "Vertex of ID: " + std::to_string(this->_id);

		if( with_edges ) {
			for( auto edge_pair : this->_edges ) {
				auto edge = edge_pair.second;
				ret += "\n\t-> Adjacent edge: " + std::to_string(edge->vertex->getId()) + "  weight: " + std::to_string(edge->weight);
			}
		}
		return ret;
	}
...
/**
 *  Class used to compare vertex indegree in priority queues
 *   This definition makes it prefer the lower indegree of the two vertices
 */
class IndegreeComparer
	{
	public:
		bool operator()(const Vertex* lhs, const Vertex* rhs)
		{
