...
{
	DijkstraVertex * vector = dijkstra_queue.top();
	vector->set_known();

	for (auto tempEdge : vector->getEdges())
	{
		auto tempEdge2 = tempEdge.second;
		if (!tempEdge2 ->dijkstra_vertex->is_known())
		{
			DijkstraVertex * adjVec = tempEdge2->dijkstra_vertex;
			double cvw = tempEdge2->weight;

			if (vector->get_path_weight() + cvw < adjVec->get_path_weight())
			{
				adjVec->set_path_weight(vector->get_path_weight() + cvw);
				adjVec->set_path_ptr(vector);
			}
			dijkstra_queue.push(adjVec);
		}
	}
	dijkstra_queue.pop();
	}
