...
class BST {
    ...
    void printLevelOrderHelper(Node<T> *root, std::ostream& out) {
        ...
        queue<Node<T> *> q; // traverse the tree in order by defining queue of type pointer
        q.push(root); //push the root inside for Testing
        //Node* out= nullptr;//somewhere
        while (!q.empty()){ // continue testing until is empty
          Node<T> * temp = q.front();//extract the front first and pop it up
          q.pop();

            //if (temp->key == key)out = temp;// if temp contains the key I am looking for save it somewhere
          if (temp->left != NULL) q.push(temp->left);// if temp->setLeft is not null throw it inside the queue
          if (temp->right != NULL) q.push(temp->right);// if temp->setRight is not null throw it inside the queue
    ...
    BST& operator=(BST & other) {
        ...
        _root = cloneTree(other._root); // copy value of the root through the object passed in
      return *this;
      }
    ...
    BST& operator=(BST && other) {
        ...
        swap (_root, other._root); //swaps the value of _root and other._root
